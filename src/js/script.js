...
$('#searchInput').on('input', function() {
    ...
    let filteredProducts = shop.products.filter(function(product) {
        if (product.title.toUpperCase().includes(filter.toUpperCase()))
            return product;
    });
    printProducts(filteredProducts);
});
...
function showProduct() {
    ...
    for (let i = 0; i < product.category.length; i++) {
        ctgrColor.push(
            shop.categories.find(({ name }) => name === product.category[i])
        );
        $('#product_details_header').prepend(
            $(
                '<h2 class="mr-1 badge modal-title badge-' +
                ctgrColor[i].color.toLowerCase() +
                '">'
            ).text(product.category[i])
        );
    }
    ...
    $('#thumb1')
        ...
        .click(changeThumb);
    $('#thumb2')
        .attr('src', 'src/img/' + idProduct + '_thumb2.jpg')
        .click(changeThumb);
    $('#thumb3')
        .attr('src', 'src/img/' + idProduct + '_thumb3.jpg')
        .click(changeThumb);
}
        ...
        $('#cart-content')
            .append(
                ...
                .append(
                    ...
                    .append(
                        $('<h6>', {
                            text: cartsProducts[j].price.toFixed(2) + '\u20ac',
                        })
                    )
                    .append(
...
function showSummary() {
    ...
    for (let j = 0; j < cartsProducts.length; j++) {
        ...
        $('#summary_products').append(
            $(emptyCol).append(
                $(
                    '<img class="img-thumbnail col-5 float-left" src="' +
                    cartsProducts[j].img +
                    '" />'
                ),
                $('<h6 class="col-7 float-right mb-0">').text(cartsProducts[j].title),
                $('<small class="col-7 float-right d-block">').text(
                    cartsProducts[j].price.toFixed(2) + ' €/pc'
                ),
                $('<small class="col-7 float-right">').text(
                    cart[j].quantity + ' ' + pieceNumber
                )
            )
        );
    }
